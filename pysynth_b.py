...
##########################################################################
 print(piano key frequency table)##########################################################################
pitchhz, keynum = {}, {}
...
def linint(arr, x):
	...
	for v in arr:
		...
		if v[0] < x and x - v[0] >= lx - x:
			lx = v[0]
			ly = v[1]		
	#print(lx, ly, ux, uy)	return (float(x) - lx) / (ux - lx) * (uy - ly) + ly


#if __name__ == '__main__':
    #print("Piano key frequencies (for equal temperament):")    #print("Key number\tScientific name\tFrequency (Hz)")for k in range(88):
    freq = 27.5 * 2.**(k/12.)
    oct = (k+9) // 12
    note = '%s%u' % (keys_s[k%12], oct)
    #if __name__ == '__main__':
    #    print("%10u\t%15s\t%14.2f" % (k+1, note.upper(), freq))    pitchhz[note] = freq
    keynum[note] = k
...
#print(harmtab[keynum['c4'],:])for h2 in range(88):
	for n in range(20):
		ref = harmtab[h2,1]
...
def make_wav(song,bpm=120,transpose=0,leg_stac=.9,boost=1.1,repeat=0,fn="out.wav", silent=False):
	...
	#print("Note frequencies in song:", cache_this)	data = np.zeros((repeat+1)*t_len + 441000.)
	#print(len(data)/44100., "s allocated")
	for rp in range(repeat+1):
		for nn, x in enumerate(song):
		    if not nn % 4 and silent == False:
		        print("[%u/%u]\t" % (nn+1,len(song)))		    if x[0]!='r':
		        if x[0][-1] == '*':
		            vol = boost
		            note = x[0][:-1]
		        else:
...
if __name__ == '__main__':
	print("*** EXPERIMENTAL PIANO VERSION WITH NOTE CACHING ***")	print
	print("Creating Demo Songs... (this might take about a minute)")	print
